...
class BinnedPRInsight(Insight):
    ...
    def __init__(self, table_name, db_conninfo, key, bins, filter = None, order = None, remove_uncertain=True, joins=[]):
        super().__init__(table_name, db_conninfo, remove_uncertain)
        self.key = key
        self.bins = bins
        self.filter = filter
        self.order = order
    ...
    def refresh(self):
        ...
        for filter_name, filter_where in [('all', 'TRUE'), ('popular', 'stars > 10')]:
            ...
            for agent, popularity_bin, popularity_min, total_prs, closed_prs, merged_prs in self.cursor.fetchall():
